...
**Definition: (Heteroskedasticity variance estimator)** When Heteroskedasticity is true, define the estimator of the variance of $\hat\beta$ as:
$$
\hat V(\hat\beta|X) = (X'X)^{-1}X'S X(X'X)^{-1}
$$
where $S = \hat\Sigma = diag[\hat e_i^2]$

**Definition: (Homoscedasticity variance estimator)** When Homoscedasticity is true, define the estimator of the variance of $\hat e$ as:
$$
...
3. $E[\hat V(\hat\beta|X)] = Var(\hat\beta|X)$
...
2. $\hat e |X \sim N(\beta ,\sigma^2M_X)$
3. $\hat \beta$ is independent to $\hat e$
4. $(n-k)s^2/\sigma^2 \sim \chi^2(n-k)$
5. $\hat \beta$ is independent to $s^2$
6. $\hat T|X=\frac{\hat \beta -\beta}{\sqrt{s^2[(X'X)^{-1}]_{jj}}}|X \sim T(n-k)$
7. When $C$ is a $1\times k$ vector, we have $\hat T'|X=\frac{C\hat \beta -C\beta}{\sqrt{s^2C(X'X)^{-1}C'}}|X \sim T(n-k)$
8. When $R$ is a $J \times k$ matrix, we have $F|X=\frac{(R(\hat \beta-\beta ))'(R(X'X)^{-1}R')^{-1}(R(\hat\beta - \beta))}{\sigma^2}|X \sim \chi^2(J)$
9. When $R$ is a $J \times k$ matrix, we have $\hat F|X=\frac{(R(\hat \beta-\beta ))'(R(X'X)^{-1}R')^{-1}(R(\hat\beta - \beta))/J}{s^2}|X \sim F(J,n-k)$
